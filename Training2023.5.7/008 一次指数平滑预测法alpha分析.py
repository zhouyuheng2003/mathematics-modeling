...
for kth in range(0,7):#6
    ...
    for ap in range(0,101):
        ...
        resDenominator=0
        for i in range(1,178):
            S.append(alpha*Y1[i]+(1-alpha)*S[i-1])
        for i in range(1,178):
            if S[i]<0:
                S[i]=0
        ...
        # plt.show()
        #print(resNominator)
        f.write(str(resNominator))
        f.write(",")
        xalpha.append(alpha)
        yalpha.append(resNominator)
    fig, ax = plt.subplots(figsize=(6, 6))
    plt.xlabel('alpha ')
    plt.ylabel('RMSE')
    ax.plot(xalpha,yalpha)
    plt.show()
    f.write("\n")
    #print("\n")
