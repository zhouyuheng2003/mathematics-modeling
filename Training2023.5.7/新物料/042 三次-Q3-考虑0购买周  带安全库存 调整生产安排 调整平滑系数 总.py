...
for kth in range(0,6):#6
    ...
    ###############
    f=open(objCurrent+".csv", 'w',encoding='gbk')
    f.write("周,生产计划/件,实际需求量/件,库存量/件,缺货量/件,服务水平\n")
    lastneedmore=0
    buythistime=0
    needmore=0
    service=0
    ...
    for i in range(1,178):
        ...
        if i!=177:
            diff=Y1[i+1]-S[i]
            if diff>100: alpha=0.9
            elif diff>50: alpha=0.8
            elif diff>10: alpha=0.6
            elif diff>-10: alpha=0.3
            elif diff>-50: alpha=0.4
            elif diff>-100: alpha=0.5
            else: alpha=0.6
        if S[i]<0:
            S[i]=0
        if i>=101-1 and i<=177-1:
            ...
            if(have>=S[i]):
                ...
                buythistime=math.ceil(S[i])+SS-have
            ...
            if Y1[i+1]==0:
                ...
                service=1.0*have/Y1[i+1]
                needmore=Y1[i+1]-have
                have=0
                serviceTot+=1
            Y3.append(have)
            ################
            f.write(str(i+1))
            f.write(",")
            f.write(str(lastneedmore+buythistime))
            f.write(",")
            f.write(str(Y1[i+1]))
            f.write(",")
            f.write(str(have))
            f.write(",")
            f.write(str(needmore))
            f.write(",")
            f.write(str(service))
            f.write("\n")
            lastneedmore=needmore
            ################
        else:
            Y3.append(0)
